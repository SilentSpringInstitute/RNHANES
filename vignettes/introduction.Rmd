...
Each time you call `nhanes_load_data`, the data will be downloaded, even if you've already downloaded it before. To save time, you can download the files to a folder so that RNHANES doesn't have to redownload them each time.

```{r, eval=FALSE}
nhanes_load_data("EPH", "2007-2008", cache = "./nhanes_data")
```

So far, we've been downloading the data without its accompanying demographic information. This information is available in a separate file for each cycle. RNHANES can automatically download the correct demography file and merge it with your data.
...
All of the following analyses will be done on the following data.
