test_that("nhanes_search works", {
  ...
  test_that("nhanes_search on variables passes spot check", {
    results <- nhanes_search(vars, "mono-ethyl", destination = destination)
  ...
  test_that("nhanes_search on files passes spot check", {
    results <- nhanes_search(files, "Polyfluoroalkyl", destination = destination)
  ...
  test_that("fuzzy search works on variables", {
    results <- nhanes_search(vars, "Perfluorooctanoic", fuzzy = TRUE, cycle == "2003-2004", destination = destination)

    expect_equal(nrow(results), 17)
  })

  test_that("fuzzy search works on files", {
    results <- nhanes_search(files, "fluoro", fuzzy = TRUE, destination = destination)

    expect_equal(nrow(results), 10)
    expect_equal(results$data_file_name, c("SSANA_A", "SSANA_B", "SSANA_C", "SSPFC_A", "L24PFC_C", "PFC_D", "PFC_E", "PFC_F", "PFC_G", "PFC_POOL"))
  })
})
